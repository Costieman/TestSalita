(() => {
  ...
  const fn = name => globalValue(name) || window[name];
  ...
  const level = () => typeof fn("levelInfo") === "function" ? n(fn("levelInfo")()?.level || 1) : n(at("level","learnerLevel")) || 1;
  const xp = () => typeof fn("totalLearningPoints") === "function" ? n(fn("totalLearningPoints")()) : n(at("xp","totalXp","learningPoints"));
  ...
  const totalAvatars = () => n(window.SalitaAvatarModel?.catalogue?.length || window.SalitaAvatarModel?.all?.()?.length || 0);
  ...
  function install() {
    const list = catalogue();
    if (!Array.isArray(list) || !appState()) {
    ...
    try { fn("syncEarned")?.({bootstrap:true}); fn("renderCatalogue")?.(); } catch {}
  }
